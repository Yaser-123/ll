...
export default function ChatScreen() {
  ...
  const targetPeerId = peerId ?? BROADCAST_ID;
  const conversationId = makeConversationId(deviceId, targetPeerId);
  const conversation = getConversation(conversationId);
  
  const selectedPeer = getPeerList().find((p) => p.id === peerId);
  const peerName = selectedPeer ? selectedPeer.displayName : 'Broadcast (All)';
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      {peerId && (
        <View style={styles.peerHeader}>
          <Text style={styles.peerHeaderText}>Chatting with <Text style={{fontWeight: 'bold'}}>{peerName}</Text></Text>
        </View>
      )}
